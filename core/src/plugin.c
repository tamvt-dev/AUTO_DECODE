...
// Define PluginManager struct BEFORE any function that uses it
...
static PluginManager *current_manager = NULL;
...
static void register_plugin_builtin(const char *name,
    ...
    p->decode_single = decode_single;
    p->decode_multi = decode_multi;
    p->encode = encode;
    p->detect = detect;
    p->priority = priority;
    p->enabled = TRUE;
